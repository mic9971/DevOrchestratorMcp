...
4. Trust review contracts only when the comment author matches the Plan Issue author in Phase 2.
5. Compare GitHub second-precision comment timestamps with the task submission timestamp normalized to one second; comments from earlier seconds are stale, while a comment created in the same second as submission remains eligible.
...
Both tools are safe for the Codex allow-list because they consume GitHub-authored contracts rather than granting Codex the direct privileged `review_submit` tool. For strict separation of duties, do not give Codex GitHub Issue-comment write credentials.
