...
internal static class OrchestratorModel
{
    public static void Configure(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<DevelopmentTask>(entity =>
        {
            ...
            entity.HasMany(x => x.Dependencies).WithOne().HasForeignKey(x => x.TaskId).OnDelete(DeleteBehavior.Cascade);
            entity.HasMany(x => x.Evidence).WithOne().HasForeignKey(x => x.TaskId).OnDelete(DeleteBehavior.Cascade);
            entity.HasMany(x => x.Reviews).WithOne().HasForeignKey(x => x.TaskId).OnDelete(DeleteBehavior.Cascade);
            entity.HasMany(x => x.Events).WithOne().HasForeignKey(x => x.TaskId).OnDelete(DeleteBehavior.Cascade);

            entity.Metadata.FindNavigation(nameof(DevelopmentTask.AcceptanceCriteria))!.SetPropertyAccessMode(PropertyAccessMode.Field);
            entity.Metadata.FindNavigation(nameof(DevelopmentTask.Dependencies))!.SetPropertyAccessMode(PropertyAccessMode.Field);
            entity.Metadata.FindNavigation(nameof(DevelopmentTask.Evidence))!.SetPropertyAccessMode(PropertyAccessMode.Field);
            entity.Metadata.FindNavigation(nameof(DevelopmentTask.Reviews))!.SetPropertyAccessMode(PropertyAccessMode.Field);
            entity.Metadata.FindNavigation(nameof(DevelopmentTask.Events))!.SetPropertyAccessMode(PropertyAccessMode.Field);
        });
